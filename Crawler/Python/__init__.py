...
def GetNotScanUrls():
    ...
    result_set = session.query(db.Pages.id, db.Pages.url).filter(
        db.Pages.last_scan_date == None).all()
    return result_set
...
for site in sites:
    ...
    for sitemap_url in sitemaps_urls:
        ...
        for url in urls:
            bullet = db.Pages(url=url, site_id=site.id,
...
# Так по каждой странице, т.е мы по одной странице проходимся со всеми
# личностями и условиями парсинга, и берем следующую, даные о поиске
# заносим в таблицу PersonPageRank
for url in urls:
    html_file = Downloader.download_html(url.id, url.url)
    for query in queryes:
        count = Parser.html_parse(query.keyword_1, query.keyword_2,
                                  query.distance)
        if count:
            bullet = db.PushToPersonPageRank(rank=count, page_id=url.id,
